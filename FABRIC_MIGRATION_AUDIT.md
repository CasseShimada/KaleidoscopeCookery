...
- 最后更新时间：2026-07-22 19:07（Asia/Shanghai，1.1.0.7 正式 Release 说明、唯一版本保留策略与远端清理全部完成）
- 执行模型：Codex（GPT-5）
- 当前分支：`26.2-fabric`
- 当前功能提交：`5ca9e75c0a2f53d9d10a7c2afb45f6a77bc68832`（`feat: add three-mod API compatibility`）
- 正式发布标签目标：`e85f1acbc3140912bbbc3dab61f8b96fa0f9b9cf`（`1.1.0.7-fabric+mc26.2`）；`26.2-fabric` 在其后仅有发布自动化/审计提交，不改变该正式资产
...
| WTHIT | 20.0.0 正式插件使用当前 Fabric loader 实际支持的 `waila_plugins.json` common/client 双入口；11 个服务端数据项和 2 个纯方块状态项共 13 个 feature config | VERIFIED | 远端 API JAR 以大小和 SHA-256 固定；6 类存储、烤肉架、砧板、油壶、磨盘和食谱方块只经 `IDataProvider` 同步显示所需数据，客户端 provider/磨盘 redirect 独立注册。专服 99/99、三模组组合 108/108，三模组客户端共同加载时 Cookery common/client plugin 均初始化并完成资源/图集/声音启动；默认运行类路径不含 WTHIT/Bad Packets |
...
| Carry On | 同时提供 `carryon:block_whitelist`、`carryon:block_blacklist` 与硬边界 `c:relocation_not_supported`；集成测试只直接调用公开 API 1 | VERIFIED | 用户提供的本地 26.2 / 2.9.1 API 开发树和 JAR 已实际加载。31 个单方块持久 BE（9 个设备/容器、11 椅、11 桌）在 blacklist/whitelist 两种模式均通过 `requestPickupBlock`/`requestPlace` 往返，逐项保持完整 BE NBT 与非朝向状态，并验证 attachment 的提交和清除；85 个不安全补集硬拒绝，磨盘、蒸笼、烤肉架、3x3 冷盘在世界改变前拒绝。默认 99/99、blacklist 安装态及三模组组合 108/108、whitelist 定向 1/1 通过 |
| Diggus Maximus | 使用公开 API 1 和 `included_blocks`/`excluded_blocks` 标签；不注册无作用监听器或 groups | VERIFIED | 仅油块和稻草块允许连锁挖掘，其余 114 个方块（含全部 Carry On 安全容器及除两种惰性块外的不安全补集）拒绝。真实三连油块 `requestExcavation` 精确破坏/掉落 3 个；含 NBT 的锅拒绝且数据不变；九块磨盘从边缘请求拒绝且结构完整。隔离 99/99 与三模组组合 108/108 通过 |
...
| `compat/overlay`、六个 Jade 服务端 provider、`compat/wthit` common/client 源码与 `waila_plugins.json` | 抽取防御性物品快照，加入 WTHIT 20 的 13 项正式 overlay、最小服务端数据、客户端组件和磨盘中心重定向；保留 Jade 旧翻译别名 | VERIFIED |
| `data/diggusmaximus/**`、`data/c/tags/block/relocation_not_supported.json` | 以公开标签将 Diggus Maximus 限定到两种惰性块，并将 Carry On 的 85 个不安全方块提升为硬搬运边界 | VERIFIED |
| `src/compatTest/**`、三个 `*CompatTestAccess` 与 GameTest | 本地 opt-in source set 直接编译公开 API 1 桥；默认测试只反射本项目桥且先检查 mod ID，避免可选模组缺失时提前解析 API 类 | VERIFIED |
| `build.gradle`、`gradle.properties`、`fabric.mod.json`、`scripts/verify_optional_compat.py` | 固定远端 WTHIT API 构件，隔离三个 opt-in runtime，保持 `suggests` 可选关系并锁定标签/入口/翻译/服务端边界 | VERIFIED |
| `.github/workflows/gradle-publish-26.2.yml` | 分支继续发布 timestamp prerelease；匹配版本标签时校验 `mod_version` 和唯一主 JAR，并在全构建/GameTest/验证脚本成功后发布正式非 draft、非 prerelease Release | VERIFIED |
...
| 2026-07-22 | 四仓库初始状态与 Cookery 修改前基线 | 成功 | Cookery `26.2-fabric@7b3664538852a655553f9b1d759c5975eee1a162`，除用户提供的未跟踪 prompt 外 clean；Diggus Maximus `26.2@b1adb033849190b373baa8943e034005476404e2` 与 Carry On `26.2@be1702a255940c5e29b1c48203a4ee08adc11839` 均保留用户未提交 API/文档/测试开发内容；WTHIT `dev/26.2@4af37f91e2e5c7a838b7e201753214cecaad222a` 仅有其未跟踪迁移 prompt。修改前执行 `.\gradlew.bat clean runDatagen build runGameTest --warning-mode all --console=plain`，97/97 required tests、build/datagen 成功 | VERIFIED |
| 2026-07-22 | 三个参考 API 源码/文档/示例/测试与可用构件审计 | 成功 | Diggus Maximus tag `v1.5.9-beta.1+26.2` / API 1，本地 JAR 142,279 bytes、SHA-256 `BD0AE571B9C40AC2564774A1AB2F6640F007F21AD082254A4FF603136D4FA098`；Carry On tag `v2.9.1+mc26.2` / API 1，本地 JAR 489,299 bytes、SHA-256 `15976DCD6940C03D614161CBDD60911E6DE9BF2DF5D8E115F72922594C1B584A`；WTHIT branch 20.0.0 API JAR 118,219 bytes、SHA-256 `1056477E0AC7E9FC1882422927CB795F905038FC784FD3F8BE70DD5914E81138`，runtime JAR 880,900 bytes、SHA-256 `7816A72B854349663C0D9FF9DD74A3811820BD5696EACBEF0BA2FB7682E76C99`。三个参考工作树均未修改 | VERIFIED |
| 2026-07-22 | WTHIT 隔离专服/客户端 smoke | 成功 | `.\gradlew.bat runGameTest -PwthitCompatSmoke=true --console=plain` 为 99/99；真实 loader 初始化 Cookery common plugin，公开 API probe 验证 13 个 feature config、11 个 BE data target，以及果篮 3 个苹果和砧板 2/5 的最小 namespaced payload，无完整 BE NBT。`.\gradlew.bat runClientGameTest -PclientCompatGameTests=true -PwthitCompatSmoke=true --console=plain` 正常结束；common/client plugin 均初始化，资源、OpenGL、OpenAL 与主要图集完成 | VERIFIED |
| 2026-07-22 | Diggus Maximus API 1 隔离行为测试 | 成功 | `.\gradlew.bat runGameTest -PdiggusMaximusCompatJar=..\DiggusMaximus\build\libs\diggusmaximus-1.5.9-beta.1+26.2.jar --console=plain` 为 99/99；三连油块经公开 `canStartDefault`/`requestExcavation` 精确破坏并以玩家自动拾取加实体掉落合计 3 个，含 NBT 的锅与九块磨盘边缘请求均拒绝且世界/NBT/结构不变 | VERIFIED |
| 2026-07-22 | Carry On API 1 blacklist/whitelist 行为测试 | 成功 | blacklist 配置下 `.\gradlew.bat runGameTest -PcarryOnCompatJar=..\carryon\build\libs\carryon-fabric-26.2-2.9.1.jar --console=plain` 连同 Carry 自带 9 项为 108/108。whitelist 配置使用 `fabric-api.gametest.filter=kaleidoscope_cookery_gametest:kaleidoscope_cookery_game_tests_carry_on_round_trip_preserves_supported_cookery_data` 定向运行 Cookery 测试为 1/1；该项内部逐个真实搬起/放下全部 31 个安全方块并检查 attachment、完整持久 NBT 和非朝向状态，同时在改变世界前拒绝四类代表性不安全结构。Carry 自带三项假定 blacklist 的 vanilla 夹具不纳入 whitelist 定向运行 | VERIFIED |
...
| 2026-07-22 | Release 说明、唯一版本保留策略与防复发清理 | 成功 | 正式 Release 标题统一为 `Kaleidoscope Cookery 1.1.0.7 — Fabric for Minecraft 26.2`，正文补齐三模组功能、依赖边界、全部测试结果、运行环境、Actions/提交链接与资产 SHA-256。用户随后明确要求远端除本次版本外一个不留，覆盖此前不删除旧项的限制：已删除 1 个未发布 draft、2 个旧正式 Release、6 个 snapshot Release，以及 2 个旧正式标签和 52 个 snapshot 标签。最终 GitHub API 与 `git ls-remote --tags` 均只返回 `1.1.0.7-fabric+mc26.2`，其 Release ID `357945507`、tag 与 `targetCommitish` 仍精确指向 `e85f1ac...`，唯一资产大小/digest 不变。提交 `47b4439aa77c7d8b63ebcc107517fe4962b1b82d` 将 branch build 改为保留 14 天的 `actions/upload-artifact@v4`，不再创建 snapshot Release/tag；run [`29914171739`](https://github.com/CasseShimada/KaleidoscopeCookery/actions/runs/29914171739) 完整成功，上传单一 artifact 且正式发布步骤按 branch ref 跳过 | VERIFIED |
...
1.1.0.7 的本地三模组适配与发布输入已完成验证：默认 99/99、三模组组合 108/108、Carry On whitelist 定向 1/1、WTHIT 三模组客户端共同加载，以及 12/12 静态脚本全部成功。远端分支和版本标签验证结果在本节后续行及对应 GitHub Actions/Release 中继续作为权威发布证据；不得用本地 JAR 代替标签 Actions 资产。
...
60. `VERIFIED`：1.1.0.7 的 WTHIT 生产适配只依赖 GitHub Release 20.0.0 中固定大小/SHA-256 的 API JAR；WTHIT runtime 与 Bad Packets 只在 `wthitCompatSmoke=true` 时进入运行环境。发布 JAR 和默认 runtimeClasspath 均不含两者。
61. `VERIFIED`：Diggus Maximus 和 Carry On 当前 API 1 开发内容尚未作为远端 Maven API 发布，因此只有 `src/compatTest` 的行为测试通过显式本地 JAR property 编译；生产兼容完全由公开数据标签完成，不需要这些本地 JAR，默认和 GitHub Actions 构建可在全新 runner 重现。两个用户工作树的未提交内容保持原样。
62. `VERIFIED`：三模组组合未发现互相注册、重复 overlay、客户端类越过专服边界、搬运/连锁挖掘数据复制或丢失。已知唯一测试编排限制是 Carry On 自带三个 vanilla GameTest 将 blacklist 模式写为前提，故 Cookery 的 whitelist 行为用 Fabric 精确 test selector 单独运行；该 Cookery 测试本身覆盖全部 31 个安全方块和四类硬拒绝结构。

## 1.1.0.7 发布冻结规则

本地代码、版本和主 JAR 输入已经冻结；功能分支 Actions run `29912633321` 与精确 tag ref Actions run `29913071529` 均已成功。唯一保留的远端版本标签保持指向包含发布前完整审计的 `e85f1acbc3140912bbbc3dab61f8b96fa0f9b9cf`，并与 `mod_version` 完全一致为 `1.1.0.7-fabric+mc26.2`；正式资产已由该 tag ref 的 Actions 重建并上传。后续 workflow/审计提交不改变版本代码或正式 JAR，且不得移动既有标签回填。用户已明确将发布保留策略改为只保留本次版本，旧 draft/Release/snapshot/tag 已清理；branch CI 此后只使用短期 Actions artifact。不得手工替换正式资产或修改三个参考仓库。用户原世界和启动器实例不在本轮部署范围，继续保持不动。

## 给后续模型的接手摘要

1.1.0.7 已正式发布并作为远端唯一版本保留。标签 `1.1.0.7-fabric+mc26.2` 与正式 run `29913071529` 精确指向 `e85f1acbc3140912bbbc3dab61f8b96fa0f9b9cf`；Release 非 draft、非 prerelease、为 latest stable，唯一主 JAR 为 5,309,809 bytes / 5,246 entries / SHA-256 `D530AFFB38F842480ADF9CD8247FC01904ABDEEDED9D8FA1A2F721315B0DA76A`，GitHub digest、隔离下载与内部版本一致。默认 99/99、Diggus 隔离 99/99、WTHIT 专服 99/99、Carry blacklist 与三模组组合 108/108、Carry whitelist 定向 1/1、三模组客户端共同加载、12/12 静态脚本均已验证。远端现只有这一项 Release 与标签；branch CI 经 `47b4439a` / run `29914171739` 改为短期 Actions artifact，不再生成 snapshot。三个参考仓库、用户世界、启动器和外部进程均未修改。
