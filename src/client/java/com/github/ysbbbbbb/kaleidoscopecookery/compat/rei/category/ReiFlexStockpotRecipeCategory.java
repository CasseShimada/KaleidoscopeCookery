...
public class ReiFlexStockpotRecipeCategory implements DisplayCategory<ReiFlexStockpotRecipeCategory.FlexStockpotRecipeDisplay> {
    public static final CategoryIdentifier<FlexStockpotRecipeDisplay> ID =
            CategoryIdentifier.of(KaleidoscopeCookery.MOD_ID, "plugin/flex_stockpot");
    private static final Identifier BG = Identifier.fromNamespaceAndPath(
            KaleidoscopeCookery.MOD_ID, "textures/gui/jei/stockpot.png");
    ...
    public List<Widget> setupDisplay(FlexStockpotRecipeDisplay display, Rectangle bounds) {
        ...
        widgets.add(Widgets.withTranslate(Widgets.createDrawableWidget((guiGraphics, mouseX, mouseY, delta) ->
                drawCenteredString(guiGraphics, Component.translatable("jei.kaleidoscope_cookery.flex_recipe"),
                        WIDTH / 2, 90)), startX, startY));
    ...
    public static void registerDisplays(DisplayRegistry registry) {
        for (RecipeHolder<FlexStockpotRecipe> holder :
                ClientRecipeLookup.getRecipes(ModRecipes.FLEX_STOCKPOT_RECIPE)) {
            ...
            EntryIngredient carrier = recipe.carrier().isEmpty()
                    ? EntryIngredient.empty()
                    : ReiUtil.ofIngredient(recipe.carrier());

            EntryIngredient soupBaseEntry = EntryIngredient.empty();
            ISoupBase soupBase = SoupBaseManager.getSoupBase(recipe.soupBase());
            if (soupBase == null) {
                KaleidoscopeCookery.LOGGER.warn("Skipping missing soup base {} in REI flex stockpot recipe {}",
                        recipe.soupBase(), holder.id().identifier());
            } else {
            ...
            registry.add(new FlexStockpotRecipeDisplay(holder.id().identifier(), inputs, outputs,
                    carrier, soupBaseEntry));
        }
    }
    ...
    public static class FlexStockpotRecipeDisplay extends BasicDisplay {
        ...
        public FlexStockpotRecipeDisplay(Identifier location, List<EntryIngredient> inputs,
                                         List<EntryIngredient> outputs, EntryIngredient carrier,
                                         EntryIngredient soupBase) {
        ...
        @Override
        public DisplaySerializer<? extends FlexStockpotRecipeDisplay> getSerializer() {
